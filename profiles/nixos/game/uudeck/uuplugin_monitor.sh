...
get_mount_point() {
  ...
  while true; do
    ...
    if [ ! -z "$(df -k | grep -m 1 -E "[ \t]+$mount_point[ \t]*$" | grep -v 'grep')" ]; then
      break
    fi
...
download() {
  ...
  local file="$2"
  local plugin_info=$(
    curl -L -s -k -H "Accept:text/plain" "${url}" ||
...
check_plugin_upgrade() {
  ...
  local latest_plugin_info=$(
    curl -L -s -k -H "Accept:text/plain" "${DOWNLOAD_URL}" ||
